...
public class BakedModelEditableWall extends BakedModelEditable {
	...
	public BakedModelEditableWall(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter,
		...
		this.baked_post = ModelUtil.makeCube(format, 0.25, 0, 0.25, 0.5, 1, 0.5, null, texes, -1);
		this.baked_west = ModelUtil.makeCube(format, 0, 0, 0.3125, 0.5, 0.875, 0.375, null, texes, -1);
		this.baked_east = ModelUtil.makeCube(format, 0.5, 0, 0.3125, 0.5, 0.875, 0.375, null, texes, -1);
	}
	...
	public void addGeometry(List<BakedQuad> quads, EnumFacing side, IBlockState state, TextureAtlasSprite[] texes, int tintIndex) {
		...
		post = ModelUtil.makeCube(this.format, 0.25, 0, 0.25, 0.5, 1, 0.5, null, texes, tintIndex);
		north = ModelUtil.makeCube(this.format, 0.3125, 0, 0, 0.375, 0.875, 0.5, null, texes, tintIndex);
		south = ModelUtil.makeCube(this.format, 0.3125, 0, 0.5, 0.375, 0.875, 0.5, null, texes, tintIndex);
		west = ModelUtil.makeCube(this.format, 0, 0, 0.3125, 0.5, 0.875, 0.375, null, texes, tintIndex);
		east = ModelUtil.makeCube(this.format, 0.5, 0, 0.3125, 0.5, 0.875, 0.375, null, texes, tintIndex);
		if (up) {
	...
	public void addItemModel(List<BakedQuad> quads, EnumFacing side) {
		this.baked_post.addToList(quads, side);
		this.baked_east.addToList(quads, side);
